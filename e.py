...
claro = unpad(decipher_aes.decrypt(ciphertext), BLOCK_SIZE)
hash_object = SHA256.new()
hash_object.update(claro)
if hash_object.digest() == hash:
    print(claro.decode("utf-8", "ignore"))
...
try:
    ...
    print(datos.decode("utf-8", "ignore"))
...
cipher_AT = DES.new(K_AT,DES.MODE_ECB)
cifrado_K_AT = cipher_AT.encrypt(pad(M.encode("utf-8"),BLOCK_SIZE_8))
...
decipher_AT = DES.new(K_AT,DES.MODE_ECB)
descifrado_K_AT = unpad(decipher_AT.decrypt(msg_K_AT), BLOCK_SIZE_8)
cipher_BT = DES.new(K_BT,DES.MODE_ECB)
cifrado_K_BT = cipher_BT.encrypt(pad(descifrado_K_AT,BLOCK_SIZE_8))
...
decipher_BT = DES.new(K_BT,DES.MODE_ECB)
M= unpad(decipher_BT.decrypt(msg_K_BT),BLOCK_SIZE_8).decode("utf-8", "ignore")
print(M)
...
h1=HMAC.new(K_AB,msg=M.encode("utf-8"),digestmod=SHA256)
HMAC_M = h1.digest()
...
engineRSADescifrado = PKCS1_OAEP.new(private)
Texto_M = engineRSADescifrado.decrypt(msg_Cipher_M)
h2=HMAC.new(K_AB,msg=Texto_M,digestmod=SHA256)
try:
    h2.verify(msg_HMAC_M)
    print(Texto_M.decode("utf-8"))
except (ValueError, TypeError):
    print("HMAC invalido")